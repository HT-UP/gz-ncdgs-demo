export type Tenant = {
  id: string
  name: string
  type: '设计单位' | '监理单位' | '施工单位' | '运营单位' | '研究机构' | '政府部门'
  status: '启用' | '停用'
  ldap: boolean
  sso: boolean
  storageQuota: number
...
const tenantBase = [
  { id: 't1', name: '广州地铁设计研究院', type: '设计单位' as const },
  { id: 't2', name: '中铁监理一分公司', type: '监理单位' as const },
  { id: 't3', name: '广州盾构工程公司', type: '施工单位' as const },
  { id: 't4', name: '轨道运营管理公司', type: '运营单位' as const },
  { id: 't5', name: '华南理工大学课题组', type: '研究机构' as const },
  { id: 't6', name: '市交通运输局', type: '政府部门' as const },
  { id: 't7', name: '南方测绘工程公司', type: '施工单位' as const },
  { id: 't8', name: '设备集成安装公司', type: '施工单位' as const },
]
...
export function createTenants(count: number): Tenant[] {
...
export function createPolicies(count: number): AccessPolicy[] {
  const now = new Date('2026-08-12T10:00:00')
  const subjectPool = ['数据中心管理员', '业务分析员', '数据治理专员', '审计员', '张三', '李四']
  const statusPool = ['生效', '生效', '待生效', '已过期', '待审批'] as const
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      actions: ['读', '全部', '写'][index % 3] as AccessPolicy['actions'],
      level: ['高', '中', '低'][index % 3] as AccessPolicy['level'],
...
export function createAuditLogs(count: number): AuditLogItem[] {
  const now = new Date('2026-08-12T10:00:00')
...
export function createMaskingRules(count: number): MaskingRule[] {
  const now = new Date('2026-08-12T10:00:00')
  const methodPool = ['掩码', '掩码', '替换', '哈希'] as const
  return Array.from({ length: count }, (_, index) => {
    const date = new Date(now)
    ...
    return {
      ...
      status: ['已上线', '已上线', '已上线', '审批中', '草稿'][index % 5] as MaskingRule['status'],
...
export function createComplianceItems(count: number): ComplianceItem[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      ...
      status,
      riskLevel: ['高', '中', '低'][index % 3] as ComplianceItem['riskLevel'],

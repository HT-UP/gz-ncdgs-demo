...
const statusPool = ['已发布', '已发布', '待审核', '草稿', '已废止'];
...
export function createDataElements(count) {
...
export function createCollectionTasks(count) {
    const now = new Date('2026-08-11T14:32:08');
    ...
    const statusPool = ['成功', '成功', '运行中', '失败', '等待调度'];
    return Array.from({ length: count }, (_, index) => {
        const date = new Date(now);

...
export function createPermissionApplications(count) {
    const now = new Date('2026-08-12T10:00:00');
    const statusPool = ['已批准', '待审批', '待审批', '已驳回', '已到期'];
    const permPool = ['读', '写', '全部'];
    return Array.from({ length: count }, (_, index) => {
        const date = new Date(now);
...
export function createFeatureResults(count) {
    const semanticPool = ['姓名', '手机号', '身份证号码', '日期', '金额', '编码', '地址', '通用文本'];
    const rolePool = ['主键', '业务字段', '业务字段', '外键', '业务字段'];
    const rulePool = [
        '枚举值校验：status ∈ {0,1,2}',
        '取值范围：18 ≤ age ≤ 100',
...
export function createExploreTasks(count) {
    const now = new Date('2026-08-12T10:00:00');
    const triggerPool = ['手动触发', '定时调度', '事件触发'];
    const statusPool = ['成功', '成功', '运行中', '失败', '待执行'];
    return Array.from({ length: count }, (_, index) => {
        const date = new Date(now);

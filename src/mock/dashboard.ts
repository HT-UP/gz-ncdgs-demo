export type DashboardStat = {
  key: 'dataSourceTotal' | 'standardTotal' | 'qualityScore' | 'taskTotal' | 'auditVol' | 'auditIssue' | 'auditFixed' | 'auditRate'
  label: string
  value: string
  trend: string
  routeHint: string
...
const createDates = (count: number) => {
...
const randomShift = (base: number, delta = 2) => base + Math.round((Math.random() - 0.5) * delta)

export function createDashboardMock(): DashboardMockData {
  const now = new Date()

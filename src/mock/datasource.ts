export type DataSourceCategory = '结构化' | '半结构化' | '非结构化'

export type DataSourceStatus = '已连接' | '连接中' | '已断开' | '异常'
...
const typePool: { type: string; category: DataSourceCategory }[] = [
...
const statusPool: DataSourceStatus[] = ['已连接', '已连接', '已连接', '连接中', '已断开', '异常']
...
export function createDataSources(count: number): DataSourceItem[] {

export type DevTaskStatus = '运行中' | '成功' | '失败' | '待执行'

export type DevTaskType = '批量' | '实时' | '流式'
...
const statusPool: DevTaskStatus[] = ['成功', '成功', '运行中', '失败', '待执行', '成功']
const typePool: DevTaskType[] = ['批量', '批量', '批量', '实时', '流式']
...
export function createDevTasks(count: number): DevTask[] {

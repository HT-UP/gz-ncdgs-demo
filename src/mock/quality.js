...
const typePool = ['完整性', '准确性', '一致性', '唯一性', '及时性'];
const priorityPool = ['高', '中', '低'];
const statusPool = ['启用', '启用', '启用', '停用', '草稿'];
...
export function createQualityRules(count) {
...
export function createQualityTasks(count) {
    const now = new Date('2026-08-11T14:32:08');
    const triggerPool = ['周期调度', '周期调度', '实时触发', '手动执行'];
    const schedulePool = ['每日 03:00', '每小时', '实时', '手动'];

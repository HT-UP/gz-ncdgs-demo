...
export function createAITasks(count) {
    ...
    return Array.from({ length: count }, (_, index) => {
        ...
        return {
            ...
            type: ['元数据补全', '分类分级', '质量规则生成', '知识入库', '智能问答', '报告生成'][index % 6],
...
export function createKnowledgeEntries(count) {
    const now = new Date('2026-08-12T10:00:00');
    const titlePool = [
        '《数据安全法》要点解读',
        '个人信息脱敏处理规范',
...
export const agentExecutionPool = {
    meta: {
        ...
        statusPool: ['成功', '成功', '成功', '失败'],
    },
    quality: {
        actions: ['规则自动执行', '实时规则监控', '专项方案制定', '质量报告生成'],
        statusPool: ['成功', '成功', '运行中', '成功'],
    },
    resource: {
        actions: ['资产智能分析', '目录信息确认', '目录回写', '资产归档'],
        statusPool: ['成功', '运行中', '成功', '失败'],
    },
    search: {
        actions: ['语义解析', '相关性检索', '关系视图生成', '问答回复'],
        statusPool: ['成功', '成功', '成功', '成功'],
    },
};
export function createAgentLogs(agentKey, count) {
